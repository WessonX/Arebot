...
import os,subprocess,socket,time,signal,urllib
from arebot_service.srv import RobotService, RobotServiceResponse
import rospy
import json,yaml
...
def handle_robot_service(req):
    global gmap,nav,tf,cam,wall,enco,program,programName
    rospy.loginfo("Get Requset: %s",req.name)
    ...
    if req.name=="startGmapping":
       ...
       # os.system('rosnode kill /amcl /joint_state_publisher /ls01b_v2 /map_server /move_base /robot_state_publisher /smart_car /tf_odom_publish')
        gmap= subprocess.Popen(['roslaunch arebot_slam slam.launch &'],shell=True, close_fds=True, preexec_fn = os.setsid,stdout=subprocess.PIPE)
        ...
        if os.system('cd $(rospack find arebot_navigation)/maps && rosrun map_server map_saver -f ./%s'%req.arg)==0:
       ...
       # os.system('rosnode kill /amcl /joint_state_publisher /ls01b_v2 /map_server /move_base /robot_state_publisher /smart_car /tf_odom_publish')
        if req.arg=='':
            ...
            nav= subprocess.Popen(['roslaunch arebot_navigation navigation.launch map_name:=%s'%req.arg],shell=True, close_fds=True, preexec_fn = os.setsid,stdout=subprocess.PIPE)
            tf= subprocess.Popen(['rosrun tf2_web_republisher tf2_web_republisher &'],shell=True, close_fds=True, preexec_fn = os.setsid,stdout=subprocess.PIPE)
            time.sleep(5)
        if nav!=0 and tf!=0:
            print 1
        ...
        path=os.popen('rospack find arebot_navigation').read()[:-1]
        if req.arg=='':
            if(os.path.isdir("%s/maps"%path)):
                return RobotServiceResponse("findMap ok!",0,os.listdir("%s/maps"%path))
            else:
    ...
    elif(req.name=='runProgram'):
        path=os.popen('rospack find arebot_service').read()[:-1]+"/scripts/BlockPrograms/"
        #　将未执行完毕程序终止
        if program!=0:
            os.killpg(os.getpgid(program.pid),signal.SIGINT)
            rospy.loginfo("Program %s process has been killed",programName)
        #  保存程序到当前路径
        programName = path + "blockControl.py"
        programContent = urllib.unquote(req.arg)
        # rospy.loginfo("runProgram params: %s",programContent)
        with open(programName, "w") as fp:
            fp.write(programContent)
        rospy.loginfo("Create program %s", programName)    
        #   执行程序
        command = 'python '+programName
        rospy.loginfo("Run program %s", programName)    
        program = subprocess.Popen([command],shell=True, close_fds=True, preexec_fn = os.setsid,stdout=subprocess.PIPE)
        #   获取程序执行状态
        status = program.poll()
        if status == 0:
            resmsg = "Program completed successfully!"
            rescode = 0
            program = 0
        elif status == None:
            resmsg = "Program still running!"
            rescode = 1
        else:
            resmsg = "Program execution failed!"
            rescode = 2
            program = 0
        rospy.loginfo("runProgram resonse:{{msg: %s, code: %d, list: []}}",resmsg,rescode)
        return RobotServiceResponse(resmsg,rescode,[])
    elif(req.name=='stopProgram'):
        if program!=0:
            os.killpg(os.getpgid(program.pid),signal.SIGINT)
            rospy.loginfo("Program %s process has been killed",programName)
            program = 0
            rospy.loginfo("stopProgram resonse:Program terminated!,0,[]")
            return RobotServiceResponse("Program terminated!",0,[])
        else:
            rospy.loginfo("stopProgram resonse:{{msg: No program running!, code: 1, list:[]}}")
            return RobotServiceResponse("No program running!",1,[])
    elif(req.name=='checkProgramStatus'):
        if program!=0:
            #   获取程序执行状态
            status = program.poll()
            rospy.loginfo("Program %s running status: %s",programName,status)
            if status == 0:
                resmsg = "Program completed successfully!"
                rescode = 0
                program = 0
            ...
            resmsg = "No program running!"
            rescode = 3
        rospy.loginfo("checkProgramStatus resonse:{{msg: %s, code: %d, list: []}}",resmsg,rescode)
        return RobotServiceResponse(resmsg,rescode,[])

    
def robot_service_server():
    rospy.init_node('robot_service_server')  
    s = rospy.Service('arebot_service', RobotService, handle_robot_service)
